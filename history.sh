...
PROMPT_COMMAND="${PROMPT_COMMAND:+$PROMPT_COMMAND ; }"'printf "%q %q %b\n" "$USER@$HOSTNAME" "$(readlink -e -- "$PWD")" "$(history 1 | cut -d " " -f4-)" >> ~/.bash_all_history'
...
if [ "$1" ]
then
    grep "$(printf '%q' "$(readlink -e -- "$PWD")")" ~/.bash_all_history | grep "$1"
else
    grep "$(printf '%q' "$(readlink -e -- "$PWD")")" ~/.bash_all_history
fi
}
